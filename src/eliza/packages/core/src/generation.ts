...
// verifiableInferenceOptions,
{
    ...
    try {
        ...
        switch (provider) {
            ...
            case ModelProviderName.ZEROG_COMPUTE: {
                elizaLogger.info("Initializing ZEROG Compute.");
                ...
                const wallet = new ethers.Wallet(privateKey, zgProvider);

                // Initialize broker
                const broker: ZGComputeNetworkBroker = await createZGComputeNetworkBroker<ZGComputeNetworkBroker>(
                    wallet,
                    undefined,
                    undefined,
                    undefined,
                ...
                elizaLogger.info(`accountDetails balance: ${accountDetails.ledgerInfo[0]}`);
                elizaLogger.info(`accountDetails spend: ${accountDetails.ledgerInfo[1]}`);

                if (!accountDetails.ledgerInfo[0]) {
                    await broker.ledger.addLedger(1);
                    // await broker.ledger.depositFund(0.1, 13000000000);
                }
                ...
                // elizaLogger.info(`services:-- ${services as ServiceStructOutput}`);  
                
                let providerAddress;

                const zModel = runtime.getSetting("ZEROG_COMPUTE_MODEL");
                elizaLogger.info(`runtime.getSetting("ZEROG_COMPUTE_MODEL"): ${zModel}`)
                ...
                switch (zModel || "") {
                    ...
                    case "deepseek-r1-70b":
                        ...
                        break;
                    default:
                        providerAddress = "0x960E74Fc0AF1a6fBcADA3eEFCBe3152fA5E87A5f";
                }
                ...
                elizaLogger.info(`endpoint:model: ${endpoint}:${model}`);  
                ...
                await broker.inference.settleFee(providerAddress, 0.00000000000000003);
                ...
                const relModel = openai.languageModel(model);
                ...
                response = openaiResponse;
                elizaLogger.info("Received response from ZEROG Compute.");
                break;
            }
        ...
        runtime.imageModelProvider === runtime.modelProvider
            ...
            : (() => {
                  ...
                  switch (runtime.imageModelProvider) {
                      ...
                      case ModelProviderName.ZEROG_COMPUTE:
                          return runtime.getSetting("ZEROG_PRIVATE_KEY");
                      default:
...
export async function handleProvider(
    ...
    switch (provider) {
        ...
        case ModelProviderName.ZEROG_COMPUTE:
            return await handleZeroGCompute(options);
        case ModelProviderName.BEDROCK:
 ...
 * @returns {Promise<GenerateObjectResult<unknown>>} - A promise that resolves to generated objects.
 */
async function handleZeroGCompute({
    model,
    apiKey,
    schema,
    schemaName,
    ...
    const zerogcompute = createOpenAI({ 
        apiKey, 
        baseURL: models.zerog_compute.endpoint,
        fetch: runtime.fetch 
    });
    const settings = schema ? { structuredOutputs: true } : undefined;
    return aiGenerateObject({
        model: zerogcompute.languageModel(model, settings),
        schema,
        schemaName,
        schemaDescription,
        mode,
