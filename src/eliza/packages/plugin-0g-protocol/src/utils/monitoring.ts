...
/**
 * Tracks download metrics and logs them
 */
export const monitorDownload = (metrics: Omit<DownloadMetrics, 'timestamp'>): void => {
    const downloadMetrics: DownloadMetrics = {
        ...metrics,
        timestamp: new Date().toISOString()
    };

    elizaLogger.info('Download metrics', downloadMetrics);

    // Log errors if present
    if (!metrics.success && metrics.error) {
        elizaLogger.error('Download failed', {
