...
export const zgUpload: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        ...
        try {
            ...
            const config = {
                ...
                allowedExtensions: runtime.getSetting("ZEROG_ALLOWED_EXTENSIONS")?.split(",") || [".pdf", ".png", ".jpg", ".jpeg", ".doc", ".docx", ".webp"],
    ...
    handler: async (
        ...
        try {
            ...
            elizaLogger.info("uploadContext", uploadContext);
            elizaLogger.info("uploadSchema", uploadSchema);
            elizaLogger.info("ModelClass.MEDIUM", ModelClass.MEDIUM);
            ...
            elizaLogger.info("response-content", response);

            const uploadContent = response.object as UploadContent;

            // Validate upload content
            if (!isUploadContent(runtime, uploadContent)) {
                elizaLogger.info("isUploadContent");
            ...
            elizaLogger.debug("Extracted file path", { filePath, uploadContent });
            ...
            elizaLogger.info("filePath", filePath);
            elizaLogger.info("ZEROG_UPLOAD_DIR", runtime.getSetting("ZEROG_UPLOAD_DIR"));
            ...
            elizaLogger.info("validator", validator);
            // Validate file type
            ...
            elizaLogger.info("typeValidation", typeValidation);
            // Validate file size
            ...
            elizaLogger.info("pathValidation", pathValidation);
            elizaLogger.info("pathValidation.isValid", pathValidation.isValid);
            ...
            elizaLogger.info("sanitizedPath", sanitizedPath);
            ...
            try {
                ...
                const evmRpc = runtime.getSetting("ZEROG_EVM_RPC");
                ...
                console.log("evmRpc:", evmRpc);
                console.log("indexer:", indexer);
                // const provider = new ethers.JsonRpcProvider(evmRpc);
                // const privateKey = '';
                // const signer = new ethers.Wallet(privateKey, provider);
                // const snRpc = ' https://indexer-storage-testnet-turbo.0g.ai';
                // const indexerR = new Indexer(indexer.toString());
    ...
    examples: [
        [
            ...
            },
            {
                user: "{{agent}}",
                content: {
                    text: "Sure, I will help you upload the image 00001_.webp to storage.",
                    action: "ZG_UPLOAD",
                    content: {
                        filePath: "xxxxxxxx",
                    },
