...
export class MessageManager {
    ...
    private async processImage(
        ...
        try {
            ...
            const uploadDirectory = this.runtime.getSetting("ZEROG_UPLOAD_DIR")
            let uploadFilePath;
            ...
            if (imageUrl) {
                ...
                if (imageUrl.split(".")[-1] == "mp4") {
                    return { description: `${uploadFilePath}` }
                }
                ...
                if (!imageDescriptionService) {
                    ...
                    if (messageText.includes("0G")){
                        uploadFilePath = `${uploadDirectory}${messageId}-0g.jpg`
                        axios({
                            url: imageUrl,
                            method: 'GET',
                            family: 4, 
                            ...
                            const writer = fs.createWriteStream(uploadFilePath);
                            response.data.pipe(writer);
        
                            writer.on('finish', () => {
                                elizaLogger.info('Message 0g File downloaded successfully');
                              });
                            
                            writer.on('error', (err) => {
                                elizaLogger.error('Message mange Error writing file:', err);
                            });
        
                          }).catch(err => {
                            elizaLogger.error('Message 0g Error downloading file:', err);
                          });
                    } else if (messageText.includes("phala")) {
                        uploadFilePath = `${uploadDirectory}${messageId}-phala.jpg`
                        axios({
                            url: imageUrl,
                            method: 'GET',
                            family: 4,  
                            ...
                            writer.on('finish', () => {
                                elizaLogger.info('Message phala File downloaded successfully');
                              });
                            ...
                            elizaLogger.error('Message phala Error downloading file:', err);
                          });
                    } else if (messageText.includes("aws")) {
                        uploadFilePath = `${uploadDirectory}${messageId}-aws.jpg`
                        axios({
                            url: imageUrl,
                            method: 'GET',
                            family: 4, 
                            ...
                            writer.on('finish', () => {
                                elizaLogger.info('Message aws File downloaded successfully');
                              });
                            ...
                            elizaLogger.error('Message aws Error downloading file:', err);
                          });
                    } else {
                        uploadFilePath = `${uploadDirectory}${messageId}-00.jpg`
                        axios({
                            url: imageUrl,
                            method: 'GET',
                            family: 4, 
                            ...
                            writer.on('finish', () => {
                                elizaLogger.info('MessageManager File downloaded successfully');
                              });
                            
                              writer.on('error', (err) => {
                                elizaLogger.error('MessageManager Error writing file:', err);
                              });
        
                          }).catch(err => {
                            elizaLogger.error('MessageManager Error downloading file:', err);
                          });
                    }
                }    
                elizaLogger.info(`MessageManager uploadFilePath: ${uploadFilePath}`);
                return { description: `${uploadFilePath}` }
